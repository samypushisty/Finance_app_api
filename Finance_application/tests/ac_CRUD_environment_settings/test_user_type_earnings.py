...
def test_delete(client,data_for_test):
    response = client.delete("/api/v1/environment_settings/type_of_earnings", params={"token": data_for_test.token, "table_id": data_for_test.data.data["table_id"]})
    answer = json.loads(response.text)
    print(answer)
    print(data_for_test.data.status_code)
    print(time.time())
...
def test_get(client,data_for_test):
    response = client.get("/api/v1/environment_settings/type_of_earnings", params={"token": data_for_test.token, "table_id": data_for_test.data.data["table_id"]})
    answer = json.loads(response.text)
    print(answer)
    print(data_for_test.data.status_code)
    print(time.time())
