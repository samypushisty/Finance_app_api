...
from api.api_v1.services.total_balance.schemas import BalancesHistoryRead, BalanceRead, BalanceGet
...
async def get_total_balance(
        ...
        balance: BalanceGet = Depends(),
        total_balance_service: UserBalanceService = Depends(get_total_balance_service),
        ):
    return await total_balance_service.get_balance(token=token, balance=balance)
...
async def get_history_balances(
    ...
    return await total_balance_service.get_balances(token=token, balance=balance)